...
- 《马克思主义基本原理》已作为政治讲义导入书架：8章、20个真实小节、8个参考页；34个考点拆分页已合并回所属小节，考点仍完整保留为页内标题。包内图片通过受 manifest 约束的本地只读接口访问，不暴露任意文件路径。
...
- 个人解析自动保存到本地 `data/practice/`，并原地重建按学科聚合的一份练习解析；检测到 Obsidian vault 时写入 `YuReader/<领域>/<学科>/练习解析.md`，否则回退到 `data/practice-notes/`，不会生成一题一个 Markdown 文件。
