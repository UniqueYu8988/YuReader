...
书架顶层按 医学 / 政治 / 英语 三个学习领域分类，领域内为 学科 → 资料 → 章节 → 小节。既有医学书籍没有新字段时，运行时安全回退为 `domain=medicine`、`resource_type=book`、`subject=书名`，不为此批量改写正式书架包。正式目录中的练习入口遵循规则：只有存在真实题目关联（稳定知识位置）时才显示，没有真实题库样本前不放空按钮。
...
- 《核心考案·马克思主义基本原理》已作为政治讲义导入书架：8章、54个阅读页、8个参考页；包内图片通过受 manifest 约束的本地只读接口访问，不暴露任意文件路径。
...
- 练习页一次呈现一道真实题。作答前不向浏览器返回正确答案或原书解析；提交后才显示答案、原书解析与个人解析输入区。
- 个人解析自动保存到本地 `data/practice/`，并原地重建按学科聚合的一份练习笔记；检测到 Obsidian vault 时写入 `YuReader/练习笔记/<领域>/<学科>.md`，否则回退到 `data/practice-notes/`，不会生成一题一个 Markdown 文件。
...
- 章节笔记继续按稳定小节 ID 原地保存在 `data/notes/`，不会为复习复制正文或生成每日内容快照；检测到 Obsidian vault 时，同时镜像为 `YuReader/学习笔记/<领域>/<学科>/<资料>/<章节>/<小节> · <稳定ID>.md`。因此可在 Obsidian 文件树按学习结构浏览，稳定 ID 仍保证迁移与本地统计兼容。
