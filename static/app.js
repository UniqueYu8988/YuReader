...
export function bindNavigation() {
  ...
  $("oralFocusClozeToggle")?.addEventListener("click", toggleOralFocusClozeMode);
  ...
  $("practiceBack").addEventListener("click", returnFromPractice); $("subjectivePracticeBack").addEventListener("click", returnFromSubjectivePractice); $("subjectiveFinishSession").addEventListener("click", returnFromSubjectivePractice); $("subjectiveRevealReference").addEventListener("click", toggleSubjectiveReference); $("subjectiveAnswer").addEventListener("input", scheduleSubjectiveSave); $("subjectiveReflection").addEventListener("input", scheduleSubjectiveSave); $("practiceSubmit").addEventListener("click", submitPracticeAnswer); $("practiceMapToggle").addEventListener("click", togglePracticeSessionMap); $("practiceFinishSession").addEventListener("click", finishPracticeSession); $("practiceReadingFinish").addEventListener("click", finishPracticeSession); $("practiceReviewWrong").addEventListener("click", reviewFirstWrongPracticeQuestion); $("practiceReviewFlagged")?.addEventListener("click", reviewFirstFlaggedPracticeQuestion); $("practiceFlagBtn")?.addEventListener("click", () => togglePracticeFlag()); $("practiceLeaveSession").addEventListener("click", returnFromPractice); $("practicePrevious").addEventListener("click", () => { if (state.practiceIndex > 0) { state.practiceIndex -= 1; renderPracticeQuestion(); } }); $("practiceNext").addEventListener("click", () => { if (state.practiceIndex < (state.practice?.question_count || 1) - 1) { state.practiceIndex += 1; renderPracticeQuestion(); } else finishPracticeSession(); });
  document.addEventListener("keydown", handlePracticeKeydown);
  $("practicePersonalAnalysis").addEventListener("input", schedulePracticeAnalysisSave);
  $("practiceOpenNoteBtn")?.addEventListener("click", () => setPracticeNoteOpen(!state.practiceNoteOpen));
  $("togglePracticeNoteDock")?.addEventListener("click", () => setPracticeNoteOpen(!state.practiceNoteOpen));
