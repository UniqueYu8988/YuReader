...
export function medicinePracticeSection(index, type, title, defaultDescription, tone) {
  ...
  if (!state.oralFocus?.available || !subjects.length) {
    return `<section class="learning-center-section">${learningSectionHeader(index, title, description)}<div class="learning-empty"><strong>资料尚未导入</strong><span>资料入口已保留，等待本地重点资料。</span></div></section>`;
  }
  const subjectButtons = subjects.map((subject) => {
    return `<button type="button" data-oral-subject="${escapeHtml(subject.id)}" data-oral-type="${escapeHtml(type)}" title="${escapeHtml(subject.title)} · ${escapeHtml(title)}"><span><strong>${escapeHtml(subject.short_title)}</strong><small>${formatInteger(subject.focus_completed)} / ${formatInteger(subject.focus_count)}</small></span><i data-lucide="arrow-up-right"></i></button>`;
  }).join("");
  return `<section class="learning-center-section learning-practice-section ${tone}">${learningSectionHeader(index, title, description)}<div class="learning-subject-actions">${subjectButtons}</div></section>`;
}
...
export function renderMedicineCenter() {
  ...
  return `<section class="learning-center-section">${learningSectionHeader(1, "书架", `${books.length} 本口腔教材 · 最近阅读自动置前`, "medicine-books")}${learningRailHtml("medicine-books", books, (book) => learningBookCard(book, recentId))}</section>
    ${medicinePracticeSection(2, "definition", "名词解释", "资料入口已保留，等待本地重点资料", "basic")}
    ${medicinePracticeSection(3, "essay", "论述", "资料入口已保留，等待本地重点资料", "advanced")}`;
}